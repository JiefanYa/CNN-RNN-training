...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (2)
        wordemb_out, wordemb_cache = word_embedding_forward(captions_in, W_embed)
        # (3)
        if self.cell_type == "rnn":
            rnn_out, rnn_cache = rnn_forward(wordemb_out, affine_out, Wx, Wh, b)
        elif self.cell_type == "lstm":
            rnn_out, rnn_cache = lstm_forward(wordemb_out, affine_out, Wx, Wh, b)
        else:
        ...
        # Backward path
        # (4)
        drnn_out, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dscores,scores_cache)
        # (3)
        if self.cell_type == "rnn":
            dwordemb_out, daffine_out, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(drnn_out, rnn_cache)
        elif self.cell_type == "lstm":
            dwordemb_out, daffine_out, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(drnn_out, rnn_cache)
        else:
            raise ValueError('Invalid cell_type "%s"' % self.cell_type)
        # (2)
        grads['W_embed'] = word_embedding_backward(dwordemb_out, wordemb_cache)
        # (1)
